...
export const createAppointment = asyncErrorHandler(async (req, res, next) => {
  const { doctorId, patientId, date, time, consultationFee } = req.body;
  ...
  const discountPercentage = process.env.DISCOUNT_PERCENTAGE; //  discount for first-time consultations
  const { discountedFee, discountApplied, discount_message } =
...
export const getDoctorAppointments = asyncErrorHandler(
  async (req, res, next) => {
    const { id } = req.params;

    const appointments = await Doctor.find({ doctorId: id }).populate({
...
export const updateAppointmentStatus = asyncErrorHandler(
  async (req, res, next) => {
    ...
    // return an error if appointment not found
    if (!appointment) {
